...
class DDRProductsSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        //find category named 'RAM memory'
        $category = Category::where('name', 'RAM memory')->first();

        //traversing the above products data
        foreach($productData as $data){
            ...
            //traversing current $data's sku field, and create the skus for current product
            foreach($data['skus'] as $sku){
                $product->skus()->create(array_merge($sku, ['stock' => 999]));
            }

            //traversing current $data's properties field, and create the properties for current product
